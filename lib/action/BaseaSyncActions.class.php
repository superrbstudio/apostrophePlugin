...
result in an error when it happens to be empty. Move along, nothing to see here.");
  ...
  // have an action to ask Apache to clean up permissions making Apache-writable folders
  // world-writable again. (Note that the original "everything is world everything"
  ...
  {
    $dirs = array(aFiles::getWritableDataFolder(), sfConfig::get('sf_upload_dir'), sfConfig::get('sf_cache_dir'));
    $fs = new sfFilesystem();
    foreach ($dirs as $dir)
    {
      $fs->chmod($dir, 0777);
