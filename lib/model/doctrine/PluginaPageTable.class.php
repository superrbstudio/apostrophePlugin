...
class PluginaPageTable extends Doctrine_Table
{
  ...
  static public function getPagesInfo($options)
  {
    ...
    $withTags = sfConfig::get('app_a_navigationWithTags');

    $query = "SELECT p.id, p.slug, p.view_is_secure, p.view_guest, p.view_admin_lock, p.edit_admin_lock, p.archived, p.lft, p.rgt, p.level, p.engine, p.template, s.value AS title ";
    if ($withTags) {
      $query .= ", t.name AS tag_name ";
    }
    $query .= "FROM a_page p ";
    if ($withTags) {
      $query .= "LEFT JOIN tagging tg ON tg.taggable_model = \"aPage\" AND tg.taggable_id = p.id
        LEFT JOIN tag t ON tg.tag_id = t.id ";
    }
    $query .= "LEFT JOIN a_area a ON a.page_id = p.id AND a.name = 'title' AND a.culture = $escCulture
    ...
    foreach ($resultSet as $result)
    {
      if ($withTags)
      {
        if (!isset($tagsById[$result['id']]))
        {
          $tagsById[$result['id']] = array();
        }
        if ($result['tag_name'])
        {
          if (!isset($tagsById[$result['id']]))
          ...
          $tagsById[$result['id']][] = $result['tag_name'];
        }
      }
      // Careful: with the new LEFT JOINs on access rights and tags we have extra rows.
    ...
    if ($withTags)
    {
      foreach ($results as &$result)
      {
        if (isset($tagsById[$result['id']]))
        {
          $result['tags'] = $tagsById[$result['id']];
        }
      }
    }
    return $results;
