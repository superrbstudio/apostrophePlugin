...
{ 
  $(selector).each(function()
  {
    ...
    var timeinput = $(this);
    var picker = $('<input class="a-timepicker" />');
    var options;
    var originalId = $(this).attr('id');
    var id;
    if (originalId)
    {
      // predictable ids are easier to bind things to, if they are unique enough
      id = originalId + '_timepicker';
    }
    else
    {
      id = selector + '-timepicker-' + (Math.floor(Math.random() * 999999));
    }
    var optionsId = 'options-' + (Math.floor(Math.random() * 999999));
