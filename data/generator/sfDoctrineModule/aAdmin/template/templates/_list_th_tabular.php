<?php foreach ($this->configuration->getValue('list.display') as $name => $field): ?>
	...
	<?php if ($field->isReal()): ?>
	  [?php if ('<?php echo $name ?>' == $sort[0]): ?]
	    [?php echo link_to(__('<?php echo $field->getConfig('label') ?>', array(), '<?php echo $this->getI18nCatalogue() ?>'), '@<?php echo $this->params['route_prefix'] ?>?sort=<?php echo $name ?>&sort_type='.($sort[1] == 'asc' ? 'desc' : 'asc')) ?]
	  ...
	  [?php else: ?]
	    [?php echo link_to(__('<?php echo $field->getConfig('label') ?>', array(), '<?php echo $this->getI18nCatalogue() ?>'), '@<?php echo $this->params['route_prefix'] ?>?sort=<?php echo $name ?>&sort_type=<?php echo (substr($name, 0, 4) === 'tag_') ? 'desc' : 'asc' ?>') ?]
	  [?php endif; ?]
	<?php else: ?>
