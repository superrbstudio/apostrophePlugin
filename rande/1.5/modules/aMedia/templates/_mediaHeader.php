...
<?php if ($page->admin): ?>
	...
	<div class="a-ui a-admin-header">
	  <ul class="a-ui a-controls a-admin-controls">
	      ...
	      <?php $typeLabel = aMediaTools::getBestTypeLabel() ?>
		    <li><a href="<?php echo url_for("@a_media_other?action=resume&add=1") ?>" id="a-media-add-button" class="a-btn icon big a-add"><span class="icon"></span><?php echo a_('Add  ' . $typeLabel) ?></a></li>
	      <li><a href="<?php echo url_for("@a_media_other?action=searchServices") ?>" class="a-btn icon big a-search"><span class="icon"></span><?php echo a_('Search Services') ?></a></li>
			<?php endif ?>
		</ul>
		<?php if (aMediaTools::getOption('linked_accounts') && aMediaTools::userHasAdminPrivilege()): ?>
			<a href="<?php echo url_for("@a_media_other?action=link") ?>" class="a-btn icon a-users lite mini a-align-right a-media-link-accounts alt"><span class="icon"></span><?php echo a_('Linked Accounts') ?></a>
		<?php endif ?>
	</div>
	...
	<?php if (aMediaTools::isSelecting()): ?>
		<?php a_js_call('apostrophe.mediaClearSelectingOnNavAway(?)', url_for('@a_media_other?action=clearSelecting')) ?>
	<?php endif ?>
