...
<?php if ($single): ?>
  ...
  <ul class="a-ui a-controls a-align-left bottom">
		<?php if ($editVideoSuccess): ?>
	   	<li><?php echo a_button(a_('Cancel'), url_for('@a_media_other?action=resume'), array('icon','a-cancel', 'big')) ?></li>
		...
		<?php else: ?>
	   	...
	   	<li><?php echo a_button(a_('Cancel'), url_for('@a_media_other?action=resume'), array('icon','a-cancel')) ?></li>
		<?php endif ?>
  	<?php if ($item): ?>
    	<li>
