  ...
  <ul class="a-ui" id="a-media-video-search-results">
    <?php foreach ($pager->getResults() as $result): ?>
      <li>
        ...
        <?php echo link_to(a_('Select%buttonspan%', array('%buttonspan%' => '<span></span>')), '@a_media_other?action=editVideo&' . http_build_query(array('first_pass' => 1, 'a_media_item' => array('_csrf_token' => md5(sfConfig::get('sf_csrf_secret') . session_id()), 'view_is_secure' => 0, 'embed' => $result['url']))), array('class' => 'a-media-search-select a-btn')) ?>
