...
<?php if ((!$slot->isNew()) && isset($variants[$slot->type]) && count($variants[$slot->type])): ?>
  <li class="a-controls-item variant" id="<?php echo "$name-$permid-variant" ?>">
		<?php echo jq_link_to_function('Options', '$("#'.$name.'-'.$permid.'-variant-options-toggle").parent().toggleClass("open")', array('class' => 'a-variant-options-toggle a-btn icon a-settings', 'id' => $name.'-'.$permid.'-variant-options-toggle', )) ?>
    <ul class="a-variant-options dropshadow">
      <?php foreach ($variants[$slot->type] as $variant => $settings): ?>
        ...
        <li id="<?php echo $id ?>-inactive" class="inactive" style="<?php echo (!$active) ? '' : 'display: none' ?>">
          <?php echo jq_link_to_remote($settings['label'], array('url' => url_for('a/setVariant?' . http_build_query(array('id' => $page->id, 'name' => $name, 'permid' => $permid, 'variant' => $variant))), 'update' => "a-slot-content-$name-$permid"), array('class' => 'a-btn icon a-unchecked',)) ?>
        </li>
      <?php endforeach ?>
    </ul>
  </li>
