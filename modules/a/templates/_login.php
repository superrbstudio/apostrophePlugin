...
<?php if ($sf_user->isAuthenticated()): ?>
	...
	<li id="a-logged-in-as"><?php echo __('You are logged in as') ?> <span>								
	...
	<?php else: ?>
		<li id="a-logged-in-as"><?php echo __('You are logged in as') ?> <span><?php echo $sf_user->getGuardUser()->getUsername() ?></span></li>									
	<?php endif ?>
	
  <li><?php echo link_to(__('Log Out'), sfConfig::get('app_a_actions_logout', 'sfGuardAuth/signout'), array('class' => 'a-btn', )) ?></li>
<?php else: ?>
  <?php // You can easily turn off the 'Log In' link via app.yml ?>
  <?php if (sfConfig::get('app_a_login_link', true)): ?>
    <li>
			<?php echo jq_link_to_function(__('Login'), "$('#a-login-form-container').fadeIn(); $('#signin_username').focus(); $('.a-page-overlay').fadeIn('fast');", array('class' => 'a-btn','id' => 'a-login-button')) ?>	
