...
<?php if ($editable): ?>
  ...
  <?php slot("a-slot-controls-$pageid-$name-$permid") ?>
			<?php if ($options['image']): ?>
  			<?php include_partial('aImageSlot/choose', array('action' => 'aButtonSlot/image', 'buttonLabel' => a_get_option($options, 'chooseLabel', __('Choose Image', array(), 'apostrophe')), 'label' => a_get_option($options, 'browseLabel', __('Select an Image', array(), 'apostrophe')), 'class' => 'a-btn icon a-media', 'type' => 'image', 'constraints' => $options['constraints'], 'itemId' => $itemId, 'name' => $name, 'slug' => $slug, 'permid' => $permid)) ?>				
			<?php endif ?>
			<?php include_partial('a/simpleEditWithVariants', array('pageid' => $pageid, 'name' => $name, 'permid' => $permid, 'slot' => $slot, 'page' => $page, 'controlsSlot' => false, 'label' => a_get_option($options, 'editLabel', __('Edit', array(), 'apostrophe')))) ?>
  <?php end_slot() ?>
